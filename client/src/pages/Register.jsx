...
const Register = () => {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            const res = await axios.post('http://localhost:5000/api/users/register', formattedData);
                ...
                <form onSubmit={handleRegister} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="relative">
                            ...
                            <input name="name" placeholder="Full Name" onChange={handleChange} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200/80 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none text-sm transition-all" required />
                        </div>
                        <div className="relative">
                            <Mail className="absolute left-3.5 top-[13px] text-slate-400" size={16} />
                            <input name="email" type="email" placeholder="Email Address" onChange={handleChange} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200/80 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none text-sm transition-all" required />
                        </div>
                    </div>

                    <div className="relative">
                        <Lock className="absolute left-3.5 top-[13px] text-slate-400" size={16} />
                        <input name="password" type="password" placeholder="Secure Password" onChange={handleChange} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200/80 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none text-sm transition-all" required />
                    </div>

                    <div className="relative">
                        <BookOpen className="absolute left-3.5 top-[13px] text-slate-400" size={16} />
                        <input name="subjects" placeholder="Subjects (comma separated: React, Python, UI Design)" onChange={handleChange} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200/80 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none text-sm transition-all" required />
                    </div>

                    <div className="relative">
                        <PenTool className="absolute left-3.5 top-[13px] text-slate-400" size={16} />
                        <textarea name="bio" placeholder="Briefly tell us about your active engineering or study goals..." onChange={handleChange} className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200/80 rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none text-sm h-24 resize-none transition-all"></textarea>
                    </div>
